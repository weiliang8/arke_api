...
exports.createUser = async (req, res, next) => {
    ...
    if(err.name=="ValidationError"){
        ...
        Object.keys(err.errors).forEach(field => {
          if (err.errors[field].kind === 'required') { //Identify type of vaildation error
            errFields.push(field.charAt(0).toUpperCase() + field.slice(1)); //Cap first letter of field
          }
        });
